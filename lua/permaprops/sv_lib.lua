...
function PermaProps.ReloadPermaProps()
    ...
    PermaProps.Data = util.JSONToTable(file.Read(game.GetMap() .. ".txt", "DATA") or "[]")

    if not PermaProps.Data or PermaProps.Data == nil then MsgC(Color(200, 150, 0), "[PermaProps] No map info found.\n") return end

    for id, data in pairs(PermaProps.Data) do
